...
public class App {
	...
	public TransportClient createClient() {
		...
		try {
			client = new PreBuiltTransportClient(settings)
					.addTransportAddress(new TransportAddress(InetAddress.getByName(HOST_NAME), 9300));
	...
	public void deleteByQuery(String filter, String filterValue) {
		BulkByScrollResponse response = new DeleteByQueryRequestBuilder(client, DeleteByQueryAction.INSTANCE)
				.filter(QueryBuilders.matchQuery(filter, filterValue)).source(INDEX_NAME).get();
	...
	public IndexResponse index() {
		...
		json.put("price", "4500");
		IndexResponse response = client.prepareIndex(INDEX_NAME, TYPE_NAME, "1").setSource(json, XContentType.JSON)
				.get();
